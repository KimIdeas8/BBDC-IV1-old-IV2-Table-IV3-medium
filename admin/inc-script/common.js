...
function bookingLimit(obj, id) {
	...
	if (isNaN(obj.value)) spanid.innerHTML = '-';
	else spanid.innerHTML = newRange;
}
...
function validateInput(form, radioMsg, checkMsg) {
	for (var e = 0; e < form.elements.length; e++) {
		...
		if (el.type == 'radio') {
			...
			else for (var r = 0; r < group.length; r++) if ((checked = group[r].checked)) break;
			if (!checked) {
				alert(radioMsg);
				el.focus();
				return false;
	...
	if (rdoObj.length > 0) {
		for (i = 0; i < rdoObj.length; i++) {
			if (rdoObj[i].checked) {
				...
				if (strTxt.indexOf('a') > 0) {
					// a  - attended
					if (strTxt.charAt(strTxt.indexOf('a') + 1) != 'b') {
						// ab - absent
...
function postBookingPDIold(varSlotCount, form, url, radioMsg, checkMsg) {
	...
	if (ct.value > 0) {
		// member has clicked on some slot, so display next screen
		form.action = url;
		form.submit();
	} else {
...
function SetCountAndToggleColorORG(obj, VarSlotCount) {
	...
	if (browser == 'Netscape') {
		...
		if (td.bgColor == cellColor) {
			td.bgColor = '#d9e8f7';
			if (ct.value == null) ct.value = ct.value + 1;
		} else {
		...
		if (td.bgColor == cellColor) {
			td.bgColor = '#d9e8f7';
			if (ct != null) ct.value = eval(ct.value + 1);
		} else {
			td.bgColor = cellColor;
			if (ct != null) ct.value = eval(ct.value - 1);
		}
...
function SetMouseOverToggleColor(obj) {
	...
	{
		var td = document.getElementById(obj);
		if (ZcellColorName(td.bgColor) == cellColor) td.bgColor = '#d9e8f7';
		else td.bgColor = cellColor;
	}
	//alert(ct.value);
}
...
function isExpiryDateValid(obj) {
	...
	if (strLength == 5) {
		...
		//from 1991-2079
		if (tempYear > 90) expiresYear = '19' + tempYear;
		else if (tempYear < 80) expiresYear = '20' + tempYear;
		...
		if (today.getTime() > expiry.getTime()) isValid = false;
	}
...
function isDate(thisdate) {
	...
	if (isNaN(b) || isNaN(d) || isNaN(f)) err = 1;
...
function isCheckBoxEmpty(objid) {
	...
	if (len == undefined) {
		...
		if (allobj.checked == true) chk = false;
	}
...
function check() {
	...
	for (var i = 0; i < this.rsize; i++) {
			...
			if (input.style.visibility != 'hidden') {
				if (!isNumber(input)) {
					span.innerText =
						...
						'\n* ' +
						fieldName +
						' can only contain number characters.';
					span.style.color = '#CC0009';
					field.style.color = '#CC0009';
					ret = false;
				}
			...
			if (input.style.visibility != 'hidden') {
				if (!isAlphaNumeric(input)) {
					span.innerText =
						...
						'\n* ' +
						fieldName +
						' can only contain alphabet and numeric characters.';
					span.style.color = '#CC0009';
					field.style.color = '#CC0009';
					ret = false;
				}
		...
		if (ret == false && firstFocus == null) if (!CheckRadioisEmpty) firstFocus = input;
	}

	if (ret == false) TblSpan.style.visibility = 'visible';
...
function formatDate1(i, delKey, direction) {
	...
	if (delKey != 9) {
		//tab
		if (delKey != 46 && delKey != 16 && !(delKey > 36 && delKey < 41)) {
			//if the delete, backspace, shift, are not the keys that caused the keyup event.
			var fieldLen = i.value.length;
			if ((delKey >= 48 && delKey <= 57) || (delKey >= 96 && delKey <= 105)) {
				if (fieldLen == 2 || fieldLen == 5) {
...
function checkMonthYear(thisdate) {
	...
	if (isNaN(b) || isNaN(d) || isNaN(d)) err = 1;
	...
	for (var c = 0; c < form.elements.length; c++)
		if (form.elements[c].id == chkID) {
			if (form.elements[c].type == 'checkbox')
				if (form.elements[c].disabled != true) form.elements[c].checked = status;
		}
}

function checkBoxes2(field, status) {
	for (i = 0; i < field.length; i++) {
		if (field[i].disabled != true) field[i].checked = status;
	}
}
...
function showFullScreen(url) {
	...
	var win = open(
		url,
		'Booking',
		"'screenX=0,screenY=0,left=0,top=0,width=' + w + ',height=' + h + 'scrollbars=yes,resizable=yes"
	);
	win.moveTo(0, 0);
...
function validateForm(form) {
	for (var e = 0; e < form.elements.length; e++) {
		...
		if (el.type.indexOf('select') != -1) {
			...
			if (group.length) {
				var checked = false;
				for (var r = 0; r < group.length; r++) if ((checked = group[r].checked)) break;
				if (!checked) {
...
function popup(popupURL) {
	var popup = window
		.open(
			popupURL,
			'win',
			'toolbar=0,location=0,directories=0,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=200,height=320,top=100,left=200'
		)
		.focus();
}
...
function checkLength(obj) {
	//if (length == obj.size)
	if (obj.value.length == obj.size) return true;
...
function confirmCancelBooking() {
	...
	if (blnResult) {
		...
		if (
			confirm(
				'Please note that there will be NO REFUND of Test Fee for cancellation of test date.\n\n\nPlease click OK to proceed or CANCEL to abort transaction.'
			) == true
		) {
			return true;
		}
...
function confirmCancelBookingTrySell() {
	...
	if (blnResult) {
		if (
			confirm(
				'Please click OK to proceed with any of the following transactions \n(i)\tTry Sell\n(ii)\tCancel\n(iii)\tTake back Try Sell\n\nOr CANCEL to abort transaction.'
			) == true
		) {
			return true;
		}
...
function confirmTestCancelBooking() {
	...
	if (cancelType == 'T')
		msg =
			'Please note that there will be NO REFUND of Test Fee, Vehicle Rental Fee and Warm up Fee for cancellation of test date.\n\nAre you sure you want to try-sell your slot ?';
...
function confirmTestCancelBooking2() {
	...
	if (cancelType == 'T')
		msg =
			'Please note that there will be NO REFUND of Test Fee and Vehicle Rental Fee for cancellation of test date.\n\nAre you sure you want to try-sell your slot ?';
	else if (cancelType == 'C')
		msg =
			'Please note that there will be NO REFUND of Test Fee and Vehicle Rental Fee for cancellation of test date.\n\nAre you sure you want to cancel your slot ?';
	else if (cancelType == 'A')
...
function confirmSubmit(templName) {
	if (event.srcElement.value.toLowerCase() == 'delete') {
		if (!confirm('Are you sure you want to delete the current template?')) return;
		else checkSpan('TblSpan2');
	}
...
function confirmSubmitPDI(templName, aspName) {
	if (event.srcElement.value.toLowerCase() == 'delete') {
		if (
			!confirm(
				'Entire Template will be deleted. Are you sure you want to delete the current template?'
			)
		)
			return;
	} else checkSpan('TblSpan2');
...
function replaceSubstring(inputString, fromString, toString) {
	...
	if (toString.indexOf(fromString) == -1) {
		...
		while (temp.indexOf(fromString) != -1) {
			...
			var toTheRight = temp.substring(
				temp.indexOf(fromString) + fromString.length,
				temp.length
			);
		...
		while (temp.indexOf(midString) != -1) {
			...
			var toTheRight = temp.substring(
				temp.indexOf(midString) + midString.length,
				temp.length
			);
			temp = toTheLeft + toString + toTheRight;
		}
...
function allTickCheckBox(tobjid, objid) {
	...
	if (allobj != undefined) {
		...
		// if there is only one checkbox
		if (len == undefined) if (allobj.checked == false) allTick = false;

		for (i = 0; i < len; i++) if (allobj[i].checked == false) allTick = false;

		if (allTick) tr.checked = true;
...
function toggleCheckBox(tobjid, objid) {
	...
	if (tr.checked) checkAll(tobjid, objid);
	else uncheckAll(tobjid, objid);
}
